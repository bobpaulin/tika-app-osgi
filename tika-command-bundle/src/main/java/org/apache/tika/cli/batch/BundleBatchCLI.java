...
package org.apache.tika.cli.batch;
...
/**
 * OSGi based Tika Batch Command Line Interface
 *
 */
public class BundleBatchCLI {

    public static String FINISHED_STRING = "Main thread in BundleBatchCLI has finished processing.";

    private static Logger logger = LoggerFactory.getLogger(BundleBatchCLI.class);
    private final Options options;
    
    private final BundleContext context;

    public BundleBatchCLI(String[] args, BundleContext context) throws IOException {
    ...
    private TikaInputStream getConfigInputStream(String[] args, boolean logDefault) throws IOException {
        ...
        if (batchConfigFile != null) {
            ...
            URL config = BundleBatchCLI.class.getResource("/tika-app-batch-config.xml");
            if (config != null) {
                is = TikaInputStream.get(
                        BundleBatchCLI.class.getResourceAsStream("/tika-app-batch-config.xml"));
            } else {
                is = TikaInputStream.get(
                        BundleBatchCLI.class.getResourceAsStream("default-tika-batch-config.xml"));
            }
        }
        return is;
    }
